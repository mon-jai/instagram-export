...
async function login(page: Page, username: string) {
  ...
  await Promise.all([
    ...
    page.evaluate(() => document.querySelector<HTMLButtonElement>('button[type="submit"]')!.click()),
  ])
  ...
  if (new URL(page.url()).pathname == "/accounts/login/two_factor") {
    ...
    await Promise.all([
      page.waitForNavigation(),
      page.evaluate(() => document.querySelector<HTMLButtonElement>("form > div:nth-child(2) > button")!.click()),
    ])
  }
